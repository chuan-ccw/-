    ...
    orders = [
        {
            ...
            "phone": r[1] or "未知", 
            "status": r[2] or "未完成", 
            "tot_price": r[3] or 0 # 確保總金額有值，避免顯示錯誤
        } 
...
def admin_order_detail(order_id):
    ...
    # 修正 SQL 查詢：移除 i.temperature，只查詢 i.topping
    cursor.execute("""
    ...
    tot_p, tot_q = 0, 0
    for r in cursor.fetchall():
        # r[7]: price, r[6]: quantity, r[5]: topping
...
def order_success():
    ...
    """, (order_id,))
    order_info = cursor.fetchone() # [0:order_id, 1:store_name, 2:tot_price, 3:phone, 4:store_id]
    ...
    # 將所有資訊以清晰的變數名稱傳給 order_success.html
    return render_template(
        "order_success.html", 
        order_id=order_info[0],       # 訂單編號
        store_name=order_info[1],     # 店家名稱
        total_amount=order_info[2],   # 總金額
        customer_phone=order_info[3], # 顧客電話
        store_id=order_info[4],       # 店家 ID
        items=items                   # 訂單明細 (供前端顯示表格)
    )
